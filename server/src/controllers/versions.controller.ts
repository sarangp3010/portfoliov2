...
export const getVersion = async (
  ...
  try {
    const version = await getVersionSnapshot(req.params.versionId as string);
    if (!version) {
  ...
  try {
    ...
    switch (contentType) {
      case "BLOG":
        ...
        break;
      case "PROJECT":
        restored = await prisma.project.update({
          where: { id: contentId },
          data: {
            title: data.title as string,
            description: data.description as string,
            longDesc: data.longDesc as string | undefined,
            techStack: data.techStack as string[],
          },
        });
        break;
      case "SERVICE":
        restored = await prisma.service.update({
          where: { id: contentId },
          data: {
            title: data.title as string,
            description: data.description as string,
      ...
      case "PROFILE":
        ...
        if (existing)
          restored = await prisma.profile.update({
